...
The documentation is published at <https://luksgrin.github.io/PyEvoMotion/>
from the `docs/` sources on every push to `prod` (workflow `.github/workflows/docs.yml`).
To build it locally:
...
The API pages are generated by autodoc from the installed extension module,
so the package has to be built first (`uv sync` does that). The Rust crate's
internal documentation is published alongside at `/rustdoc/`.
